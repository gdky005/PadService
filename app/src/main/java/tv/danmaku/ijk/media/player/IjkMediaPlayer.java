...
public final class IjkMediaPlayer extends SimpleMediaPlayer {
	...
	public void setDnsAddress(String[] values) throws IOException,
		IllegalArgumentException, SecurityException, IllegalStateException {
		_setDnsAddress(values);
	}

	private native void _setDnsAddress(String[] values) throws IOException,
		IllegalArgumentException, SecurityException, IllegalStateException;

	public void setDataCallBack(long flag) throws IllegalArgumentException, SecurityException, IllegalStateException, IOException  {
		_setDataCallBack(flag);
    }
	 ...
	 * 
	 * @param surface
	 *            The {@link Surface} to be used for the video
	...
	private static class EventHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			// logger
			// .info("handleMessage---------->" + msg.what + "--->msg.arg1 = "
			...
			case MEDIA_PLAYER_PTS_UPDATE:
				// System.out.println("handleMessage MEDIA_PLAYER_PTS_UPDATE------->"
				// + msg.arg1
				// + "---->duration = " + msg.arg2);
				player.notifyOnBufferingUpdate(msg.arg1, msg.arg2);
				return;
	...
	private static void postEventFromNative(Object weakThiz, int what,
	 ...
	 public static void method2(String file, byte buffer[],int length) {                       
         BufferedWriter out = null; 
        ...
        File fileDes = new File(file);
         ...
         try {       
        	 if (!fileDes.exists()) {
              ...
              //out = new BufferedWriter(new OutputStreamWriter(                      
              //       new FileOutputStream(fileDes, true)));                            
              //out.write(buffer,0,length);   
        	 fos=new FileOutputStream(fileDes,true);
	...
	private static void postDataFromNative(Object weakThiz, byte buffer[],int length)
	{
		...
		//method2("/sdcard/tmp/test.pcm",buffer,length);
		
		//TODO test 输出buffer前5字节
		/*if(buffer.length >= 5){
			
			Log.e("array_java", "array_java"+Integer.toHexString((int)buffer[0]));
			Log.e("array_java", "array_java"+Integer.toHexString((int)buffer[1]));
			Log.e("array_java", "array_java"+Integer.toHexString((int)buffer[2]));
			Log.e("array_java", "array_java"+Integer.toHexString((int)buffer[3]));
			Log.e("array_java", "array_java"+Integer.toHexString((int)buffer[4]));
		}*/
		
	}
	//@CallbyNative for audio data
